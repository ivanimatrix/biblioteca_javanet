...
public class GuardarLibro extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GuardarLibro</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GuardarLibro at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        LibroDTO libro = new LibroDTO(id, codigo, descripcion, cantidad, fecha_ingreso, genero);
        LibroDAOImpl libroDAO = new LibroDAOImpl();
        
        if(id > 0){
            try{
                if(libroDAO.update(libro) > 0){
                    estado = true;
                    mensaje = "Datos del libro actualizados";
                }else{
            ...
            try{
                if(libroDAO.insert(libro) > 0){
                    estado = true;
                    mensaje = "Datos del libro guardados";
                }else{
                    mensaje = "Hubo un problema al guardar los datos. Intente nuevamente";
                }
            }catch(SQLException e){
                System.out.println("Error insercion nuevo libro");
